...
use Express;
  ...
  class PDF extends \FPDF{
    ...
    public function Footer(){
      ...
      $this->Cell(150, 4, t('Overeenkomst interventies ter plaatse ') . date('Y'), 0,0,'L');
  ...
  class agreementfs extends pageController{
    ...
    public function review($date){
      ...
      // if($lang){
        $linkSign = 'https://my.abcparts.be/'.$lang.'/agreementfs/signed?date=' . $datum;
        $linkHome = 'https://my.abcparts.be/'.$lang.'/agreementfs?date=' . $datum;
      // }else{
      //   $linkSign = 'https://my.abcparts.be/nl/agreementfs/signed';
      //   $linkHome = 'https://my.abcparts.be/nl/agreementfs';
      // }
      ...
      foreach($prices as $price){
        ...
        $startfee = $price->getStartFee();
        $kmVergoeding = $price->getTransportPrice();
      }

      $this->set('linkSign', $linkSign);
      $this->set('linkHome', $linkHome);
      $this->set('lang', $lang);
      $this->set('dagtarief', $dagtarief);
      $this->set('nachttarief', $nachttarief);
      $this->set('startfee', $startfee);
      $this->set('kilometervergoeding', number_format($kmVergoeding, 2, '.', ''));
    ...
    public function signed(){
      ...
      $data['lang'] = $lang;
      $data['date'] = $_GET['date'];
      $data['year'] = date('Y', strtotime($_GET['date']));
  ...
  <h4 class="phone" style="font-family:\'Segoe UI\', \'Helvetica Neue\', Helvetica, Arial, sans-serif; font-size:24px; line-height:100%; font-weight:300; color:#777777; margin-top:0; margin-bottom:0 !important; padding:0;"><span class="mc-toc-title"><span>+32 9 380 43 50<br><a href="https://www.abcparts.be" target="_blank">www.abcparts.be</a></span></span></h4>
      ...
      $bodyContent .= '<p>' . t('De overeenkomst werd bevestigd door ') . '<strong>' . ucfirst($_POST['contact']) . '</strong>' .
      t(' op ') . '<strong>' . $datum . '</strong></p>';
      $bodyContent .= '<p>' . t('In de bijlage kan u uw overeenkomst terugvinden.').

      $this->pdf($data);
      $compName = str_replace(array('/', '-', ' ', '+', '<', '>'), '', $_POST['compName']);
      $file = $this->submit($compName, $datum);
    ...
    public function getPricesForYear($year){
      ...
      //if there are no prices for this year, then use prices of last year
      if(!is_object($entries)){
        $date = date('d-m-Y', strtotime($year));
        $lastyearts = strtotime($date .'- 1 year');
        $lastYear = date('Y', $lastyearts);
    ...
    public function submit($compName, $datum){
    ...
    public function pdf($data){
      ...
      $pdf->AliasNbPages();
      $pdf->AddPage();
      $pdf->setSubject(t('Overeenkomst interventies ') . $year);

      $pdf->SetFont('Arial', '', 10);
      $pdf->Cell(170, 4, 'Kruisem, ' . $datum, 0,1,'R');
      ...
      $pdf->Cell(0, $h, t('Geldig van %1$s tot 31/12/', $datum) . $year, 0, 1, 'L');
      $pdf->Cell(0, $h, '',0,1,'L');
      ...
      $pdf->Cell(10, $h, '');
      $pdf->Cell(7, $h, chr(149));
      $pdf->Cell(0, $h, iconv('UTF-8', 'CP1256', t('Een kilometervergoeding heen en weer')),0,1,'L');
      $pdf->Cell(10, $h, '');
      $pdf->Cell(7, $h, chr(149));
