...
echo '<nav class="navbar-abc navbar navbar-default">'; //opens the top-level menu
...
foreach ($navItems as $ni) {
  //print_r($ni);
    ...
    if ($ni->hasSubmenu) {
      echo '<a data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false" href="#" target="' . $ni->target . '" class="' . $ni->classes . '">' . $name . '</a>';
    }else{
...
echo '</ul></div></div></nav>'; //closes the top-level menu
