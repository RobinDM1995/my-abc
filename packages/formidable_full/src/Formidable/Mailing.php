...
class Mailing extends Formidable {	
	...
	public function setElements($elements, $new = false) {
		if (!$new) $this->elements = $elements;
		else $this->new_elements = $elements;
	}
		...
		if (intval($formID) != 0) {
			...
			if (is_array($elements) && count($elements)) {				
				foreach ($elements as $elementID) {
					$attachments[] = preg_replace_callback('/([0-9]+)/', array(&$this, '_replaceStringElementID'), $elementID);
				}
				$mailing['attachment_elements'] = @implode(',', $attachments);
			}
			
			$nm->initializeDependency(unserialize($mailing['dependencies']));
			$depends = $nm->getDependency('raw');
			if (!empty($depends)) {
				$dependencies = array();
				foreach ((array)$depends as $dep) {						
					...
					foreach ((array)$dep['elements'] as $element) {	
						$dependencyElements[] = array_filter(array(
							'element' => $this->new_elements[$element['element']],
			...
			$mailing['dependencies'] = serialize($dependencies);
		}
		else {
		...
		foreach ((array)$deps as $rule => $dep) {									
			...
			foreach ($dep['elements'] as $er => $ea) {
				...
				if (in_array($e->getElementType(), array('radio', 'checkbox', 'select'))) {
					...
					$options_dynamic = $e->getPropertyValue('options_dynamic');
					$options_value = $e->getPropertyValue('options_dynamic_value');
					if ($options_dynamic == 'manual') {
						$options = @array_filter((array)$e->getPropertyValue('options'));
					}	
					elseif ($options_dynamic == 'pages') {
						$options = $e->getPagesForSelection($options_value['pages']);
					}
					elseif ($options_dynamic == 'members') {
						$options = $e->getUsersForSelection($options_value['members']);
					}					
					if (is_array($options) && count($options)) {	
						foreach ($options as $i => $o) {							
							if ($options_dynamic == 'manual') {
								if (empty($o['value'])) $o['value'] = $o['name'];
								if ($e->getElementType() == 'select') $elements[$er]['options'][html_entity_decode($o['value'])] = html_entity_decode($o['value']);
								else $elements[$er]['options'][html_entity_decode($o['value'])] = $th->sanitizeFileSystem($e->getHandle()).($i+1);
							}
							elseif ($options_dynamic == 'pages') {
								if (is_object($o)) $elements[$er]['options'][$o->getCollectionID()] = $o->getCollectionID();	
							}
							elseif ($options_dynamic == 'members') {
								if (is_object($o)) $elements[$er]['options'][$o->getUserID()] = $o->getUserID();
							}
						}
					}
				}
	...
	public function checkDependencies() {			
		...
		foreach((array)$dependencies as $rule) {
			if (!empty($rule)) {					
				...
				foreach((array)$rule as $dependency) {						
					...
					} elseif (!empty($dependency['match'])) {
						$value = trim(@implode(' ', (array)$value));
						if (!empty($value) && strpos($value, @implode(' ', (array)$dependency['match'])) !== false) $check = true;	
					} elseif (!empty($dependency['not_match'])) {
						$value = trim(@implode(' ', (array)$value));
						if (!empty($value) && strpos($value, @implode(' ', (array)$dependency['not_match'])) === false) $check = true;	
					} else {
		...
		if (!in_array(true, (array)$or)) return false;
		return $send;
	}
	...
	private function prepareContent($content, $format = '') {				
		...
		if (is_array($advanced) && count($advanced)) {
			foreach ($advanced as $a) {	
				...
				$values[] = preg_quote((string)$result->{$a['callback']}());
		...
		if (is_array($attributes) && count($attributes)) {
			foreach ($attributes as $a) {
				...
				$labels[] = '/{%'.$a['handle'].'.value%}/';						
				$values[] = preg_quote((string)$result->{$a['callback']}($a['handle']));							
			}
		}
		...
		if (is_array($elements) && count($elements))  {
			foreach ($elements as $element) {				
				...
				if ($dependency && is_array($dependency) && count($dependency)) {
					foreach($dependency as $row) {													
						if (!empty($row)) {
							...
							foreach((array)$row as $dep) {			
								...
								if (is_object($el)) {									
									if (!empty($dep['value']) && count(array_intersect((array)$dep['value'], (array)$el->getValue()))) {
										$show = true;
									}
									if (!empty($dep['match'])) {
										$value = trim(@implode(' ', (array)$el->getValue()));
										if (!empty($value) && strpos($value, @implode(' ', (array)$dep['match'])) !== false) $show = true;
									}
								}	
								/*								
								if ($dep['inverse']) {
				...
				if (!$show || (trim($element->getDisplayValue()) == '' && $this->getDiscardEmpty() && !$element->isLayout())) continue;
				...
				//$values[] = preg_quote($element->getElementType()=='upload'?$element->getDisplayValueExport():$element->getDisplayValue());												
																
					
				if (!$element->isLayout()) $string .= sprintf($_format, $element->getLabel(), preg_quote($element->getDisplayValue()));
				if ($element->isLayout() && !$this->getDiscardLayout()) $string .= preg_quote($element->getInput());					
			}
