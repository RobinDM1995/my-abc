...
class Result extends Formidable {
	...
	public function getDefaultErrorText($errorCode, $args = array()) {
		// set locale for Javascript errors (SESSION variable set in block template)
		if(isset($_SESSION['localeJS'])) {
			Formidable::setlocale($_SESSION['localeJS']);
		}
		...
		switch($errorCode) {
			...
			case 'ERROR_INVALID_IBAN': $text = t('Field \'%s\' is an invalid IBAN'); break;
			...
			case 'ERROR_CAPTCHA': $text = t('Field \'%s\' is invalid or can\'t be validated'); break;
	...
	private function getCheck() {
		...
		foreach((array)$dependencies as $rule) {
			if (!empty($rule)) {
				$and = array();
				foreach((array)$rule as $dep) {
					$check = false;
					$value = (array)$this->data[$dep['element']];
					if (!empty($dep['value'])) {
						if (array_intersect((array)$dep['value'], (array)$value)) $check = true;
					} elseif (!empty($dep['match'])) {
						$value = trim(@implode(' ', (array)$value));
						if (!empty($value) && strpos($value, @implode(' ', (array)$dep['match'])) !== false) $check = true;
					} else {
	...
	public function charsAllowed($key = '') {
		...
		if (is_array($check_for) && count($check_for)) {
	...
	public function minMax($key = '') {
		...
		switch ($this->element->getPropertyValue('min_max_type')) {
			case 'words':
				...
				if (count((array)$words) < $this->element->getPropertyValue('min_value') || (count((array)$words) > $this->element->getPropertyValue('max_value') && $this->element->getPropertyValue('max_value') > 0)) {
					...
					else $this->addError('ERROR_WORDS_MINIMAL', array($this->element->getPropertyValue('min_value')));
				}
			break;
			case 'chars':
				if (!$str->min($value, $this->element->getPropertyValue('min_value')) || (!$str->max($value, $this->element->getPropertyValue('max_value')) && $this->element->getPropertyValue('max_value') > 0)) {
					...
					else $this->addError('ERROR_CHARS_MINIMAL', array($this->element->getPropertyValue('min_value')));
				}
			break;
			case 'value':
				if ($value < $this->element->getPropertyValue('min_value') || ($value > $this->element->getPropertyValue('max_value') && $this->element->getPropertyValue('max_value') > 0)) {
					...
					else $this->addError('ERROR_VALUE_MINIMAL', array($this->element->getPropertyValue('min_value')));
				}
			break;
			case 'options':
				$value = array_filter($value);
				if (count((array)$value) < $this->element->getPropertyValue('min_value') || (count((array)$value) > $this->element->getPropertyValue('max_value') && $this->element->getPropertyValue('max_value') > 0)) {
					if ($this->element->getPropertyValue('min_value') == $this->element->getPropertyValue('max_value') && $this->element->getPropertyValue('max_value') > 0) $this->addError('ERROR_OPTION_COUNT', array($this->element->getPropertyValue('min_value')));
	...
	public function iban($key = '') {
		$value = $this->getData(!empty($key)?$key:$this->element->getHandle());
		if (!empty($value)) {
			$ih = new \Concrete\Package\FormidableFull\Src\Helpers\IbanHelper($value);
			if ($ih->validate()) return false;
		}
		$this->addError('ERROR_INVALID_IBAN');
	}
