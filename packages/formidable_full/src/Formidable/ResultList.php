...
class ResultList extends DatabaseItemList {
    ...
    public function filterByKeyword($keyword) {
		...
		if (!empty($keyword) && (is_array($this->elements) && count($this->elements))) {
			foreach ($this->elements as $element) {
				$filter[] = "fas.answerSetID = (SELECT answerSetID FROM FormidableAnswers WHERE formID = :formID AND elementID = ".intval($element->getElementID())." AND answerSetID = fas.answerSetID AND answer_formated LIKE ".$keyword.")";
			}
		}
		if (count($filter)) $this->filter(false, '('.@implode(' OR ', $filter).')');
	}
