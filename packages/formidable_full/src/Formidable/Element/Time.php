	...
	public function generateInput() {	
		...
		switch ($this->getPropertyValue('appearance')) {
			...
			case 'input':				
				...
				$mask = preg_replace(array('/{qq}/', '/{ww}/', '/{ee}/', '/{rr}/'), array('00', '00', '00', 'SS'), $placeholder);
				$this->addJavascript("if ($.fn.mask) { $('#".$this->getHandle()."').mask('".$mask."', {placeholder: '".str_replace(array('00', 'SS'), array('__', 'AM/PM'), $mask)."'}) }");
			break;
