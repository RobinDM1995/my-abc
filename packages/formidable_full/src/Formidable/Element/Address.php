...
class Address extends Element {
	...
	public function generateInput() {			
		...
		if (empty($value) && $this->getPropertyValue('default_country') != '') {	
			$value = array();
			$value['country'] = $this->getPropertyValue('default_country');
		}
		
		if (empty($value) && intval($this->getPropertyValue('use_geocode')) == 1) {	
			$geolocated = Core::make(GeolocationResult::class);
			$value = array();
			$value['country'] = $geolocated->getCountryCode();
			$value['zipcode'] = $geolocated->getPostalCode();
			$value['province'] = $geolocated->getStateProvinceCode();
			$value['city'] = $geolocated->getCityName();
		}
		...
		if (strpos($this->getFormat(), '{country}')) {			
	...
	public function getDisplayValue($seperator = ' ', $urlify = true) {
		$value = $this->getValue();								
		if (!@array_filter((array)$value)) return '';
		$find = array(
			'/{n}/', 
			'/([,.:;!?])/', 
			'/{address_1}/', 
		...
		$value = str_replace(', , ', ', ', preg_replace($find, $replace, $this->getFormat()));
		return h($value);
	}
	...
	private function getCountries() {
		...
		if (intval($this->getPropertyValue('enable_custom_countries')) == 1) {
			$custom = $this->getPropertyValue('custom_countries');
			if (is_array($custom) && count($custom)) {
				foreach (array_keys($countries) as $c) {
					if (!in_array($c, $custom)) unset($countries[$c]);
				}
			}
		}
	...
	public function loadProvinceJS()
    {
		/*
        ...
        $rf = Core::make(ResponseFactoryInterface::class);
        return $rf->create(
