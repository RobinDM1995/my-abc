	...
	{				
		...
		$script = '
			if ($.fn.dropzone) {
				...
				$(\'div[id="'.$this->getHandle().'"]\', formObj'.$this->getFormID().').dropzone({ 
					...
					init: function() {
						...
						holder.attr(\'data-progress\', \'done\');
			            ...
			            this.on("removedfile", function(file) {
			            	var response = file;
			            	if (file.xhr && file.xhr.responseText.length > 0) response = $.parseJSON(file.xhr.responseText);
			            	if (response !== false) {
			            ...
			            this.on("complete", function (file) {
							if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
								$(\'.file_upload\', this.target).attr(\'data-progress\', \'done\');
		        				ff_'.$this->getFormID().'.check_uploads();
		      				}
		    			});   
			            this.on("uploadprogress", function(b, c) {
							$(\'.file_upload\', this.target).attr(\'data-progress\', \'uploading\');
			            	ff_'.$this->getFormID().'.check_uploads();
			            });
			            ...
			            this.removeButton = function() {
			           	...
			           	this.doCount = function(add) {	
							ff_'.$this->getFormID().'.check_depencencies();
			...
			foreach ($uploads as $file) {					
				...
				if (file_exists($path)) {
					...
					if (is_object($result)) {					
						...
						// Assign to Folder
						if (!empty($this->getPropertyValue('folder')) && !empty($this->getPropertyValue('folder_value'))) {
							$folder = FileFolder::getByID($this->getPropertyValue('folder_value'));
							if ($folder) {
