...
class Date extends Element {
	...
	public function generateInput() {
		...
		$year_max = FORMIDABLE_ELEMENT_YEAR_SELECT;
		$year_min = 1970;
		if ($this->getPropertyValue('min_max')) {
			$year_min = intval($this->getPropertyValue('min_value'));
			$year_max = intval($this->getPropertyValue('max_value'));
		}
		...
		switch ($this->getPropertyValue('appearance')) {
			case 'select':
				...
				// Add this class to disable the counter.
				$attribs['class'] .= ' counter_disabled';
				...
				$input .= $form->hidden($this->getHandle());
			break;
			...
			case 'input':
				...
				$mask = preg_replace(array('/d/', '/m/', '/y/'), array('00', '00', '0000'), $this->getFormat());
				$this->addJavascript("if ($.fn.mask) { $('#".$this->getHandle()."').mask('".$mask."', {placeholder: '".str_replace('0', '_', $mask)."'}) }");
			break;
			...
			case 'html5':
				$input = '<input type="date" name="'.$this->getHandle().'"  id="'.$this->getHandle().'" value="'.
					(!empty($value)?$value:'') .'" '.implode(' ', array_map(
	...
	public function getValue() {
		...
		if (!empty($value)) {
			if ($this->getPropertyValue('appearance') == 'select') {
				...
				if (is_array($date) && count($date)) $value = @implode('-', array($date['year'], $date['month'], $date['day']));
			}
		}
		return $value;
	}
	...
	private function translateValue($value = '') {
		...
		if (is_array($adv_value) && count($adv_value)) {
