...
class Integer extends Element {
	...
	public function generateInput() {	
		...
		if ($this->getPropertyValue('min_max')) {			
			...
			if ($this->getPropertyValue('min_max_type') == 'value') {
				...
				$attribs['max'] = intval($this->getPropertyValue('max_value'));
			}
			elseif ($this->getPropertyValue('min_max_type') == 'chars') {
				$attribs['min'] = 0;
				if (intval($this->getPropertyValue('min_value')) > 1) {
					$attribs['min'] = '1'.str_repeat('0', intval($this->getPropertyValue('min_value')) - 1);
				}
				$attribs['max'] = str_repeat('9', intval($this->getPropertyValue('max_value')));
			}
		}
		// HTML5
	...
	public function validateResult() {
		...
		if (strlen((string)$this->post($this->getHandle())) != 0) $val->integer();
