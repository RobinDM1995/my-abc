<?php 
	...
	$form_folder_selector = new Concrete\Core\Form\Service\Widget\FileFolderSelector();
	...
	$gl->sortBy('gID', 'asc');
	//$gl->includeAllGroups();
	$user_groups = $gl->getResults();

	$page_types = ['' => t('All page types')];
	$pts = PageType::getList(false);
	if (count($pts)) {
		foreach ($pts as $pt) {
			$page_types[$pt->getPageTypeID()] = $pt->getPageTypeDisplayName();
		}
	}
	...
	if (count($cols)) {
		foreach ($cols as $col) {
			$member_columns[$col->getColumnKey()] = $col->getColumnName();
		}
	};
	$aks = UserAttributeKey::getList();
	if (count($aks)) {
		foreach ($aks as $ak) {
			$member_columns['ak_'.$ak->getAttributeKeyHandle()] = $ak->getAttributeKeyDisplayName();
		}
	}
?>
		...
		<?php }
					...
					<?php 
				        ...
				        <div id="ccm-tab-content-properties" class="ccm-tab-content container-fluid">        
					        <fieldset class="form-horizontal">
								...
								<?php if ($element->getProperty('options')) { ?>
									<div class="form-group">
										<?php echo $form->label('options_dynamic', t('Options').' / '.t('Selectables'), array('class' => 'col-sm-3'))?>
										<div class="col-sm-9">
											<?php echo $form->select('options_dynamic', array('manual'=> t('Manual options'), 'pages' => t('Pages'), 'members' => t('Members')), $element->getPropertyValue('options_dynamic')); ?>
										</div>
									</div>
									
									<div class="element_options_manual">
												...
												<?php 
													...
													<div class="input-group option_row">
														...
														<div class="input-group-fields">	
															<?php echo $form->text('options_value['.$i.']', $opt['value'], array('placeholder' => t('Value'), 'style' => 'width: 44%; margin-right: 1%;')); ?>
															<?php echo $form->text('options_name['.$i.']', $opt['name'], array('placeholder' => t('Name'), 'style' => 'width: 55%;')); ?>
														</div>
									...
									<?php $options_dynamic_value = $element->getPropertyValue('options_dynamic_value'); ?>

									<div class="element_options_pages row">
										<div class="col-sm-3"></div>
										<div class="col-sm-9">
											<?php if (is_array($page_types) && count($page_types)) { ?>
												<div class="form-group">
													<?php echo $form->label('options_dynamic_value[pages][page_type]', t('Page Type'), array('class' => 'col-sm-2'))?>
													<div class="col-sm-10">
														<?php echo $form->select('options_dynamic_value[pages][page_type]', $page_types, @intval($options_dynamic_value['pages']['page_type'])); ?>
													</div>
												</div>
											<?php } ?>

											<div class="form-group">
												<?php echo $form->label('options_dynamic_value[pages][location]', t('Location'), array('class' => 'col-sm-2'))?>
												<div class="col-sm-10">
													<div class="options_dynamic_location">
														<?php echo $form->select('options_dynamic_value[pages][location]', array('' => t('Everywhere'), 'beneath' => t('Beneath a page')), $options_dynamic_value['pages']['location']); ?>
														<div id="options_dynamic_value[pages][location_page]">
															<div class="input-group">
																<div class="input-group-addon"><?php echo $form->label('options_dynamic_value[pages][location_page]', t('Select page'))?></div>
																<?php echo $form_page_selector->selectPage('options_dynamic_value[pages][location_page]', @intval($options_dynamic_value['pages']['location_page'])!=0?intval($options_dynamic_value['pages']['location_page']):''); ?>
															</div>
														</div>
													</div>
												</div>
											...
											<div class="form-group">
												<?php echo $form->label('options_dynamic_value[pages][alias]', t('Other options'), array('class' => 'col-sm-2'))?>
												<div class="col-sm-10">
													<div class="checkbox">
														<label>
															<?php echo $form->checkbox('options_dynamic_value[pages][alias]', 1, @intval($options_dynamic_value['pages']['alias']) != 0)?>
															<?php echo t('Display page aliasses'); ?>
														</label>
													</div>
													...
													<?php
														...
														<div class="checkbox">
															<label>
																<?php echo $form->checkbox('options_dynamic_value[pages][featured]', 1, @intval($options_dynamic_value['pages']['featured']) != 0)?>
											...
											<div class="form-group">
												<?php echo $form->label('options_dynamic_value[pages][order]', t('Order'), array('class' => 'col-sm-2'))?>
												<div class="col-sm-10">
													<?php echo $form->select('options_dynamic_value[pages][order]', ['display_asc' => t('Sitemap order'), 'display_desc' => t('Reverse sitemap order'), 'chrono_desc' => t('Most recent first'), 'chrono_asc' => t('Earliest first'), 'alpha_asc' => t('Alphabetical order'), 'alpha_desc' => t('Reverse alphabetical order'), 'modified_desc' => t('Most recently modified first'), 'random' => t('Random')], !empty($options_dynamic_value['pages']['order'])?@$options_dynamic_value['pages']['order']:'display_asc'); ?>
												</div>	
											</div>
										</div>
									</div>

									<div class="element_options_members row">
										<div class="col-sm-3"></div>
										<div class="col-sm-9">
											<?php if (is_array($user_groups) && count($user_groups)) { ?>
												<div class="form-group">
													<?php echo $form->label('options_dynamic_value[members][group]', t('Group(s)'), array('class' => 'col-sm-2'))?>
													<div class="col-sm-10">
														<?php foreach ($user_groups as $g) { ?>
															<div class="checkbox">
																<label>
																	<?php echo $form->checkbox('options_dynamic_value[members][group][]', $g->getGroupID(), in_array($g->getGroupID(), (array)$options_dynamic_value['members']['group']))?>
																	<?php echo $g->getGroupDisplayName(false)?>
																</label>
															</div>
														<?php } ?> 
														<div class="note"><?php echo t('If no group is selected, all users will be found') ?></div>               
													</div>	
												</div>
											<?php } ?>

											<div class="form-group">
												<?php echo $form->label('options_dynamic_value[members][name]', t('Option Name'), array('class' => 'col-sm-2'))?>
												<div class="col-sm-10">
													<?php echo $form->select('options_dynamic_value[members][name]', $member_columns, !empty($options_dynamic_value['members']['name'])?$options_dynamic_value['members']['name']:'uName'); ?>
													<div class="note"><?php echo t('This column value will be shown as selectable option') ?></div>
												</div>	
											</div>

											<div class="form-group">
												<?php echo $form->label('options_dynamic_value[members][skip_empty]', t('Other options'), array('class' => 'col-sm-2'))?>
												<div class="col-sm-10">
													<div class="checkbox">
														<label>
															<?php echo $form->checkbox('options_dynamic_value[members][skip_empty]', 1, @intval($options_dynamic_value['members']['skip_empty']) != 0)?>
															<?php echo t('Only show users where "Option Name" is not empty'); ?>
														</label>
													</div>
													<div class="checkbox">
														<label>
															<?php echo $form->checkbox('options_dynamic_value[members][validated]', 1, @intval($options_dynamic_value['members']['validated']) != 0)?>
															<?php echo t('Only validated users'); ?>
														</label>
													</div>
													<div class="checkbox">
														<label>
											...
											<div class="form-group">
												<?php echo $form->label('options_dynamic_value[members][order_by]', t('Order'), array('class' => 'col-sm-2'))?>
												<div class="col-sm-10">
													<div class="input-group" style="width: 100%;">
														<?php echo $form->select('options_dynamic_value[members][order_by]', $member_columns, !empty($options_dynamic_value['members']['order_by'])?$options_dynamic_value['members']['order_by']:'uName', array('style' => 'width:60%')); ?>
														<?php echo $form->select('options_dynamic_value[members][order_dir]', ['asc' => t('Ascending'), 'desc' => t('Descending')], !empty($options_dynamic_value['members']['order_dir'])?$options_dynamic_value['members']['order_dir']:'asc', array('style' => 'width:40%')); ?>
													</div>
												</div>	
											</div>
										</div>
								...
								<?php if ($element->getProperty('min_max')) { ?>
									<div id="min_max" class="form-group">
								...
								<?php if ($element->getProperty('fileset')) { ?>
									<div class="form-group">
										<?php echo $form->label('folder', t('Upload in folder'), array('class' => 'col-sm-3'))?>
										<div class="col-sm-9">
											<div class="input-group">
												<div class="input-group-addon"><?php echo $form->checkbox('folder', 1, intval($element->getPropertyValue('folder')) != 0)?></div>
												<div class="folder_selector">
													<?php echo $form_folder_selector->selectFileFolder('folder_value', $element->getPropertyValue('folder_value')); ?>
												</div>
											</div>
											<div class="note folder_note"><?php echo t('Uploads new files directly into the selected folder') ?></div>
										</div>
									</div>
										...
										<div class="col-sm-9">
											<div class="input-group">
												...
												if(is_array($element->getProperty('mask', 'formats'))) {
													$options = array();
													foreach($element->getProperty('mask', 'formats') as $value) { 
														$options[$value] = $value;
													}
													echo $form->select('mask_format', $options, $element->getPropertyValue('mask_format'));
												} else {
								...
								<?php if ($element->getProperty('country')) { 
										...
										$selected_countries = array_keys($countries);										
										if (intval($element->getPropertyValue('enable_custom_countries')) == 1) {											
											$selected_countries = (array)$element->getPropertyValue('custom_countries');
										}

									?>
									<div class="form-group">
										<?php echo $form->label('enable_custom_countries', t('Available countries'), array('class' => 'col-sm-3'))?>
										<div class="col-sm-9">
											<?php echo $form->select('enable_custom_countries', array(0 => t('All available countries'), 1 => t('Selected countries')), intval($element->getPropertyValue('enable_custom_countries'))); ?>
											<div id="enable_custom_countries">
												<div>
													<?php echo $form->label('custom_countries', t('Select the available countries')) ?>
													<?php echo $form->selectMultiple('custom_countries', $countries, $selected_countries, array('size' => 7)); ?>
												</div>
											</div>
										</div>
									</div>
									...
									<div class="form-group">
										<?php echo $form->label('default_country', t('Select the default country'), array('class' => 'col-sm-3')) ?>
										<div class="col-sm-9">
											<?php echo $form->select('default_country', array_merge(array('' => t('Choose country')), $countries), $element->getPropertyValue('default_country')); ?>
										</div>
									</div>
									
									<div class="form-group">
										<?php echo $form->label('use_geocode', t('Geocoding'), array('class' => 'col-sm-3')) ?>
										<div class="col-sm-9">
											<div class="checkbox">
												<label>
													<?php echo $form->checkbox('use_geocode', 1, intval($element->getPropertyValue('use_geocode')) != 0)?>
													<?php echo t('When enabled, the element will try to suggest the country based on the users IP address'); ?>
												</label>
											</div>
										</div>
									</div>
								...
								<?php if ($element->getProperty('errors') !== false) { ?>
									<div class="form-group">
							                ...
							                <div id="errors_div">                        
							                    ...
							                    <?php if ($element->getProperty('errors', 'empty')) { ?>
								                    <div class="input-subgroup required">
								                        ...
								                        <?php echo $form->text('error[empty]', isset($errors['empty'])?$errors['empty']:$err->getDefaultErrorText('ERROR_EMPTY'), array('placeholder' => $err->getDefaultErrorText('ERROR_EMPTY'))); ?>
								                    </div>
								                <?php } ?>
								                <?php if ($element->getProperty('errors', 'invalid_numeric')) { ?>
								                    <div class="input-subgroup email">
								                        ...
								                        <?php echo $form->text('error[invalid_numeric]', isset($errors['invalid_numeric'])?$errors['invalid_numeric']:$err->getDefaultErrorText('ERROR_INVALID_NUMERIC'), array('placeholder' => $err->getDefaultErrorText('ERROR_INVALID_NUMERIC'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('errors', 'invalid_email')) { ?>
								                    <div class="input-subgroup email">
								                        ...
								                        <?php echo $form->text('error[invalid_email]', isset($errors['invalid_email'])?$errors['invalid_email']:$err->getDefaultErrorText('ERROR_INVALID_EMAIL'), array('placeholder' => $err->getDefaultErrorText('ERROR_INVALID_EMAIL'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('errors', 'invalid_url')) { ?>
								                    <div class="input-subgroup url">
								                        ...
								                        <?php echo $form->text('error[invalid_url]', isset($errors['invalid_url'])?$errors['invalid_url']:$err->getDefaultErrorText('ERROR_INVALID_URL'), array('placeholder' => $err->getDefaultErrorText('ERROR_INVALID_URL'))); ?>
								                    </div>
							                    <?php } ?>
							                    <?php if ($element->getProperty('errors', 'invalid_date')) { ?>
								                    <div class="input-subgroup url">
								                        <?php echo $form->label('error[invalid_date]', t('Error when field is invalid (date)')) ?>
								                        <?php echo $form->text('error[invalid_date]', isset($errors['invalid_date'])?$errors['invalid_date']:$err->getDefaultErrorText('ERROR_INVALID_DATE'), array('placeholder' => $err->getDefaultErrorText('ERROR_INVALID_DATE'))); ?>
								                    </div>
							                    <?php } ?>
							                    <?php if ($element->getProperty('errors', 'invalid_time')) { ?>
								                    <div class="input-subgroup url">
								                        <?php echo $form->label('error[invalid_time]', t('Error when field is invalid (time)')) ?>
								                        <?php echo $form->text('error[invalid_time]', isset($errors['invalid_time'])?$errors['invalid_time']:$err->getDefaultErrorText('ERROR_INVALID_TIME'), array('placeholder' => $err->getDefaultErrorText('ERROR_INVALID_TIME'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('errors', 'confirmation')) { ?>
								                    <div class="input-subgroup confirmation">
								                        ...
								                        <?php echo $form->text('error[confirmation]', isset($errors['confirmation'])?$errors['confirmation']:$err->getDefaultErrorText('ERROR_CONFIRMATION'), array('placeholder' => $err->getDefaultErrorText('ERROR_CONFIRMATION'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('errors', 'allowed')) { ?>
								                    <div class="input-subgroup allowed">
								                        ...
								                        <?php echo $form->text('error[allowed]', isset($errors['allowed'])?$errors['allowed']:$err->getDefaultErrorText('ERROR_ALLOWED'), array('placeholder' => $err->getDefaultErrorText('ERROR_ALLOWED'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('min_max', 'words')) { ?>
								                    <div class="input-subgroup min_max words">
								                        ...
								                        <?php echo $form->text('error[words_between]', isset($errors['words_between'])?$errors['words_between']:$err->getDefaultErrorText('ERROR_WORDS_BETWEEN'), array('placeholder' => $err->getDefaultErrorText('ERROR_WORDS_BETWEEN'))); ?>
								                        <?php echo $form->text('error[words_minimal]', isset($errors['words_minimal'])?$errors['words_minimal']:$err->getDefaultErrorText('ERROR_WORDS_MINIMAL'), array('placeholder' => $err->getDefaultErrorText('ERROR_WORDS_MINIMAL'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('min_max', 'chars')) { ?>
								                    <div class="input-subgroup min_max chars">
								                        ...
								                        <?php echo $form->text('error[chars_between]', isset($errors['chars_between'])?$errors['chars_between']:$err->getDefaultErrorText('ERROR_CHARS_BETWEEN'), array('placeholder' => $err->getDefaultErrorText('ERROR_CHARS_BETWEEN'))); ?>
								                        <?php echo $form->text('error[chars_minimal]', isset($errors['chars_minimal'])?$errors['chars_minimal']:$err->getDefaultErrorText('ERROR_CHARS_MINIMAL'), array('placeholder' => $err->getDefaultErrorText('ERROR_CHARS_MINIMAL'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('min_max', 'value')) { ?>
								                    <div class="input-subgroup min_max value">
								                        ...
								                        <?php echo $form->text('error[value_between]', isset($errors['value_between'])?$errors['value_between']:$err->getDefaultErrorText('ERROR_VALUE_BETWEEN'), array('placeholder' => $err->getDefaultErrorText('ERROR_VALUE_BETWEEN'))); ?>
								                        <?php echo $form->text('error[value_minimal]', isset($errors['value_minimal'])?$errors['value_minimal']:$err->getDefaultErrorText('ERROR_VALUE_MINIMAL'), array('placeholder' => $err->getDefaultErrorText('ERROR_VALUE_MINIMAL'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('min_max', 'options')) { ?>
								                    <div class="input-subgroup min_max options">
								                        ...
								                        <?php echo $form->text('error[option_count]', isset($errors['option_count'])?$errors['option_count']:$err->getDefaultErrorText('ERROR_OPTION_COUNT'), array('placeholder' => $err->getDefaultErrorText('ERROR_OPTION_COUNT'))); ?>
								                        <?php echo $form->text('error[option_between]', isset($errors['option_between'])?$errors['option_between']:$err->getDefaultErrorText('ERROR_OPTION_BETWEEN'), array('placeholder' => $err->getDefaultErrorText('ERROR_OPTION_BETWEEN'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('min_max', 'files')) { ?>
								                    <div class="input-subgroup min_max files">
								                        ...
								                        <?php echo $form->text('error[files_count]', isset($errors['files_count'])?$errors['files_count']:$err->getDefaultErrorText('ERROR_FILES_COUNT'), array('placeholder' => $err->getDefaultErrorText('ERROR_FILES_COUNT'))); ?>
								                        <?php echo $form->text('error[files_between]', isset($errors['files_between'])?$errors['files_between']:$err->getDefaultErrorText('ERROR_FILES_BETWEEN'), array('placeholder' => $err->getDefaultErrorText('ERROR_FILES_BETWEEN'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('other')) { ?>
								                    <div class="input-subgroup other">
								                        ...
								                        <?php echo $form->text('error[other]', isset($errors['other'])?$errors['other']:$err->getDefaultErrorText('ERROR_OTHER'), array('placeholder' => $err->getDefaultErrorText('ERROR_OTHER'))); ?>
								                    </div>
							                    <?php } ?>
								                <?php if ($element->getProperty('errors', 'extension')) { ?>
								                    <div class="input-subgroup extension">
								                        ...
								                        <?php echo $form->text('error[extension]', isset($errors['extension'])?$errors['extension']:$err->getDefaultErrorText('ERROR_EXTENSION'), array('placeholder' => $err->getDefaultErrorText('ERROR_EXTENSION'))); ?>
								                    </div>
							                    <?php } ?>
												<?php if ($element->getProperty('errors', 'captcha')) { ?>
								                    <div class="input-subgroup captcha">
								                        <?php echo $form->label('error[captcha]', t('Error when captcha is empty or invalid')) ?>
								                        <?php echo $form->text('error[captcha]', isset($errors['captcha'])?$errors['captcha']:$err->getDefaultErrorText('ERROR_CAPTCHA'), array('placeholder' => $err->getDefaultErrorText('ERROR_CAPTCHA'))); ?>
								                    </div>
							                    <?php } ?>
												<?php if ($element->getProperty('errors', 'iban')) { ?>
								                    <div class="input-subgroup iban">
								                        <?php echo $form->label('error[iban]', t('Error when IBAN is invalid')) ?>
								                        <?php echo $form->text('error[iban]', isset($errors['iban'])?$errors['iban']:$err->getDefaultErrorText('ERROR_INVALID_IBAN'), array('placeholder' => $err->getDefaultErrorText('ERROR_INVALID_IBAN'))); ?>
								                    </div>
							                    <?php } ?>
							                    <div id="errors_content_note" class="note help-block"> <?php echo t('If the error is empty the default error will be shown.'); ?></div>
							                </div> 
											<div id="no_errors_div">
												<div class="note help-block"><?php echo t('Currently no validation on this field, so no custom errors have to be set.'); ?></div>
											</div>                                  
							            </div>
							        </div>
							    <?php } ?>
				        ...
				        <div id="ccm-tab-content-handling" class="ccm-tab-content container-fluid"> 
				        	<fieldset class="form-horizontal">            
				               ...
				               <div class="form-group">
									<?php echo $form->label('submission_update', t('Update on submission'), array('class' => 'col-sm-3'))?>
				                    <div class="col-sm-9">
				                        <div class="input-group">
				...
				<script>
					$(function() {
						...
						$("input.option_default").on('mousedown', function() {
					    ...
					    $(".element_options_manual .options_holder").sortable({
							items: ".option_row",
							handle: ".mover",
							sort: function(event, ui) {
								$(this).removeClass( "ui-state-default" );
	...
	<?php if ($task == 'bulk') { ?>	
	    <form id="elementBulkForm" method="post" action="">
            ...
            <fieldset class="form-horizontal">
				...
				<h5><?php echo t('Add values in the textarea. New line for each new option.').'<br>'.t('Use semicolon (;) to use key;value options.'); ?></h5>
