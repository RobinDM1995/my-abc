...
<?php 
	if (is_array($layouts) && count($layouts)) {
		foreach($layouts as $rowID => $row) { 
			$step = (is_object($row)&&$row->getAppearance()=='step')?true:false; ?>
			<div class="f-row <?php echo $step?'f-step':''; ?>" data-id="<?php echo $rowID; ?>" id="row_<?php echo $rowID; ?>">
						...
						<div class="clearfix">
							...
							foreach($row as $layoutID => $layout) { 
								...
								<div class="f-col <?php echo ($i==(count($row)-1)?'f-col-last':''); ?>" data-id="<?php echo $layout->getLayoutID(); ?>" style="width:<?php echo $width; ?>%">
									<div class="inner">
										<div class="overlay"></div>
										<div class="element_row_wrapper element-empty <?php echo (is_array($elements) && count($elements))?'hide':''; ?>"><em><?php echo t('Empty column'); ?></em></div>
										<?php 
										...
										<div class="ccm-popover-menu popover fade" data-search-menu="coloptions_<?php echo $layout->getLayoutID() ?>">
											...
											<div class="popover-inner">
												<ul class="dropdown-menu">													
													...
													<li><a href="javascript:ccmFormidableOpenDuplicateLayoutDialog(<?php echo $layout->getLayoutID() ?>,<?php echo intval($rowID) ?>);"><?php echo t('Duplicate') ?></a></li>
													<li class="divider"></li>
				...
				<div class="tools row-tools" data-launch-search-menu="rowoptions_<?php echo intval($rowID) ?>">
					<div class="tools-link"><a href="javascript:;">
						...
						<?php echo $step?'<b>'.t('Step: %s', $row->getLabel()).'</b>':t('Row'); ?>
					</a></div>
				</div>
				<div  class="ccm-popover-menu popover fade" data-search-menu="rowoptions_<?php echo intval($rowID) ?>">
					...
					<div class="popover-inner">
						<ul class="dropdown-menu">
							...
							<li><a href="javascript:ccmFormidableOpenDuplicateLayoutDialog(-1,<?php echo intval($rowID) ?>);"><?php echo t('Duplicate') ?></a></li>
							<li class="divider"></li>
