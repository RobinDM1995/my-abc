...
$(function() { 
	...
	$('input.createHandle').on('blur', function() {
		var target = $('input[name="'+$(this).data('target')+'"]');
		if (target) ccmFormidableCreateHandle($(this).val(), target);
	});
});
...
var ccmFormidableFormCheckSelectors = function(s) {
	...
	if ($('input[name=css]').is(':checked')) {
		...
		$('#css_content_note').slideUp();
	}

	var div = $('div[id="gdpr_div"]');
	if (parseInt($('select[name="gdpr"]').val()) == 1) div.slideDown();
	else div.slideUp();

	var div = $('div[id="permission_div"]');
	if (parseInt($('select[name="permission"]').val()) == 1) div.slideDown();
	else div.slideUp();
};
...
var ccmFormidableCreateHandle= function(name, target) {
	query_string = 'name='+name+'&ccm_token='+formidable_security_token_form;
	$.ajax({ 
		type: 'POST',
		url: tools_url+'handle',
		data: query_string,
		dataType: 'json',
		beforeSend: function () {
			ccmFormidableLoadLoader(true);
		...
		success: function(ret) {
			//ccmFormidableSetMessage(ret.type, ret.message);
			ccmFormidableLoadLoader();
			target.val(ret.handle);
		}
	});	
};