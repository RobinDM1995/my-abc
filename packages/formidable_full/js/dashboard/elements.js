...
var ccmFormidableFormElementCheckSelectors = function(s) {
	...
	if ($('input[name="default_value"]').is(':checked')) {
		...
		if ($('select[name="default_value_type"]').val() == 'value') {			
			$('div[id="default_value_type_value"]').slideDown().find('input[name="default_value_value"], textarea[name="default_value_value"]').attr('disabled', false);
			...
			if ((s && s.attr('name') == 'default_value_type') || (s && s.attr('name') == 'default_value')) {
				...
				$('select[name="default_value_attribute"], input[name="default_value_request"]').val('').attr('disabled', true);
			}			
			...
			$('div[id="default_value_type_request"]').slideDown().find('input[name="default_value_request"]').attr('disabled', false);
			...
			if ((s && s.attr('name') == 'default_value_type') || (s && s.attr('name') == 'default_value')) {
				...
				$('select[name="default_value_attribute"], input[name="default_value_value"], textarea[name="default_value_value"]').val('').attr('disabled', true);
			}			
			...
			$('div[id="default_value_type_'+selected+'"]').slideDown().find('select[name="default_value_attribute"]').attr('disabled', false);

			if ((s && s.attr('name') == 'default_value_type') || (s && s.attr('name') == 'default_value')) {
				$('select[name="default_value_attribute"]').focus();
				$('input[name="default_value_value"], textarea[name="default_value_value"]').val('').attr('disabled', true);
			}		
		...
		$('input[name="default_value_value"], div[id="default_value_type_request"], textarea[name="default_value_value"], select[name="default_value_attribute"]').val('').attr('disabled', true);
	}
	...
	if ($('input[name="chars_allowed"]').is(':checked')) {
		element.attr('disabled', false).animate({height:txt_max_height+'px'}, toggle_delay);
		if (s && s.attr('name') == 'chars_allowed') element.focus().find('option:first').attr('selected', true);
	} else {
		element.attr('disabled', true).animate({height:txt_min_height+'px'}, toggle_delay).find('option:selected').removeAttr('selected');
	}
	...
	var element = $('select[id="options_dynamic"]');
	if (element.length > 0) {
		var val = element.val();
		if (val == 'manual') {
			$('div.element_options_pages, div.element_options_members').slideUp();
			$('div.element_options_manual').slideDown();
		}
		if (val == 'pages') {
			$('div.element_options_manual, div.element_options_members').slideUp();
			$('div.element_options_pages').slideDown();
			if ($('select[name="options_dynamic_value[pages][location]"]').val() == 'beneath') $('div[id="options_dynamic_value[pages][location_page]"]').slideDown();
			else $('div[id="options_dynamic_value[pages][location_page]"]').slideUp();			
		}
		if (val == 'members') {
			$('div.element_options_manual, div.element_options_pages').slideUp();
			$('div.element_options_members').slideDown();
		}
	}
	
	if ($('input[name="option_other"]').is(':checked')) {
		$('input[name="option_other_value"]').attr('disabled', false);
		$('select[name="option_other_type"]').attr('disabled', false);
		...
		$('select[name="option_other_type"]').attr('disabled', true).val('');
	}
	
	
	if ($.find('input[name="multiple"]').length > 0) {
		if ($('input[name="multiple"]').is(':checked')) {
			$('.element_options_manual').find('input[type=radio]').each(function() {
			...
			$('.element_options_manual').find('input[type=checkbox]').each(function(i, element) {
				var label = $(this).parent('.input-group-addon');
	...
	var element = $('div.folder_selector');
	if ($('input[name="folder"]').is(':checked')) {
	...
	if ($('select[name="format"]').val() == 'other') {
		$('input[name="format_other"]').attr('disabled', false);
		if (s && s.attr('name') == 'format') $('input[name="format"]').focus();
	} else {
		$('input[name="format_other"]').attr('disabled', true).val('');	
	}

	if ( $('select[name="appearance"]').val() != 'picker' && 
		 $('select[name="appearance"]').val() != 'slider' && $('[id="element_type"]').val() != 'slider' && 
		 $('[id="element_type"]').val() != 'rating' &&
		 $('[id="element_type"]').val() != 'signature') {
		if (s && s.attr('name') == 'appearance') {
			...
			$('textarea[name="advanced_value"]').attr('disabled', true).val('');
			$('input[name="advanced"]').attr('checked', false).parents('.form-group').slideUp();				
		}
	} else {
		if ($.find('input[name="advanced"]').length > 0) {
	...
	//$('div[id="no_errors_div"]').slideUp();
	if (parseInt($('select[name="errors"]').val()) == 0) $('div[id="errors_div"]').slideUp();
	else $('div[id="errors_div"]').slideDown( function() { setTimeout( function() { if ($('div.input-subgroup:visible').length <= 0) $('div[id="no_errors_div"]').slideDown(); }, 500); });

	//$('div[id="enable_custom_countries"]').slideUp();
	if (parseInt($('select[name="enable_custom_countries"]').val()) == 0) $('div[id="enable_custom_countries"]').slideUp();
	else $('div[id="enable_custom_countries"]').slideDown();
	
	// Remove change event on country selector
	$('select[name="custom_counties[]"]').off('change');

	if ($('[id="element_type"]').val() == 'date') {
		var div = $('div[id="min_max"]');
		if ($('select[name="appearance"]').val() == 'select') div.slideDown();
		else div.slideUp();
	}	
...
var ccmFormidableFormElementAddOptions = function(s, value, name) {
	...
	var isParent = false;
	if (s.hasClass('element_options_manual')) isParent = true;
	...
	option += '</div>';
	option += '<div class="input-group-fields">';
	//if (second_text) {
		option += '<input type="text" name="options_value['+option_counter+']" value="'+value+'" style="width: 44%; margin-right: 1%;" placeholder="'+placeholder_value+'" class="form-control ccm-input-text">';
		option += '<input type="text" name="options_name['+option_counter+']" value="'+name+'" style="width: 55%;" placeholder="'+placeholder_option+'" class="form-control ccm-input-text">';
	...
	else $('.options_holder', s).append(new_option);
	...
	if (!isParent) {
		var element_options = s.closest('.element_options_manual')
...
var ccmFormidableFormElementRemoveOptions = function(s) {
	...
	var element_options_manual = s.closest('.element_options_manual');
	if ($('.option_row', element_options_manual).length > 1) s.closest('.option_row').remove();	
	if ($('.option_row', element_options_manual).length == 1) element_options_manual.find('.error').attr('disabled', true);	
};
...
var ccmFormidableLoadElements = function() {
	$.ajax({ 
		...
		success: function(html){
			...
			if (!html) ccmFormidableLoadPlaceholder(true);
			else {
	...
	$.ajax({ 
		...
		success: function(ret) {
			if (ret.type == 'error') {
				...
				var holder = $('div.element_options_manual');
				if (ret.clear == 1) $('div.element_options_manual div.option_row').remove();
				$.each(ret.options, function(i, value) {
					var name = value;
					if (value.indexOf(';') > 0) {
						value = value.split(";", 2);
						if (value.length == 1) {
					...
					ccmFormidableFormElementAddOptions(holder, name, value);
				});
				jQuery.fn.dialog.closeTop();
			}
		}
...
var ccmFormidableClearOptions = function() {
	$('div.element_options_manual div.option_row').remove();
	ccmFormidableFormElementAddOptions($('div.element_options_manual'));
}	
